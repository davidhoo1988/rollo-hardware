class keccak:
	...
	@staticmethod
	def prng_seed(seed): # maximal 200 bits seed
		for i in range(25):
	...
	@staticmethod
	def randint(a,b): #return a random int within [a,b]
		b_blen = len(bin(b))-2 # get bit-length of b	
	...
	@staticmethod
	def randint_hw(a,b): #return a random int within [0,b]
		if b == 1: # b=1 is an exception
			b_blen = 1
		else:
			b_blen = floor(log(b,2))
		pool_num = floor(96/b_blen)

		if keccak.rnd_pool == '': # if rand_pool is run-out
			rng = keccak.prng() # raw 96 bits rnd
			keccak.rnd_pool = rng[b_blen:pool_num*b_blen] # truncated (pool_num-1)*b_blen bits of rng to the rnd_pool
			output = int(rng[0:b_blen], 2)
		else: # if rand_pool has more entropy
			output = int(keccak.rnd_pool[0:b_blen], 2)
			keccak.rnd_pool = keccak.rnd_pool[b_blen:len(keccak.rnd_pool)]
		return output
